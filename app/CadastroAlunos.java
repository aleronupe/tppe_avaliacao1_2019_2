...
//		}
	...
	public Aluno[] ordenarAlunos() {
		...
		for (int i = 0; i < resposta.length - 1; i++) {
			for (int j = i; j < resposta.length; j++) {
				String matricula1 = formaMatricula(resposta, i);
				String matricula2 = formaMatricula(resposta, j);
	...
	private String formaMatricula(Aluno[] resposta, int pos) {
		return resposta[pos].getPrefixoMatricula() + "/" + resposta[pos].getSufixoMatricula();
	}
